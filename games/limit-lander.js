...
const LimitLander = {
    ...
    generateQuestion() {
        ...
        const a = document.getElementById('limit-answer');
        if (a) a.classList.add('hidden');
        if (window.app && window.app.assessment) {
            const e = 30 - this.engine.timer;
            const ac = this.engine.total > 0 ? this.engine.correct / this.engine.total : 0.5;
            this.difficulty = window.app.assessment.selectDifficulty(ac, e);
        } else {
    ...
    submit() {
        ...
        const est = this.inputValue;
        const err = Math.abs(est - actual);
        const maxE = Math.max(Math.abs(actual) * 0.5, 2);
        const acc = Math.max(0, 1 - err / maxE);
        const ok = acc >= 0.35;
        this.showingAnswer = true;
        const el = document.getElementById('limit-answer');
        el.innerHTML = ok
            ? '<span>\u2713 Limit = <strong>' + actual.toFixed(2) + '</strong> (you: ' + est.toFixed(1) + ')</span>'
            : '<span>\u2717 Limit = <strong>' + actual.toFixed(2) + '</strong> (you: ' + est.toFixed(1) + ')</span>';
    ...
    render(ctx, w, h) {
        ...
        const pad = 50, gH = h - pad * 2 - 80, gW = w - pad * 2;
        const lx = q.limitAt, xMin = lx - 4, xMax = lx + 4;
        let yMin = -2, yMax = 6;
        [-3, -2, -1, -0.5, 0.5, 1, 2, 3].forEach(d => {
            const y = q.fn(lx + d);
            if (isFinite(y)) { if (y < yMin) yMin = y - 1; if (y > yMax) yMax = y + 1; }
        });
        yMin = Math.max(yMin, -10); yMax = Math.min(yMax, 10);
        const sx = x => pad + (x - xMin) / (xMax - xMin) * gW;
        const sy = y => pad + (1 - (y - yMin) / (yMax - yMin)) * gH;
        ...
        // Approach arrows
        const t = Math.min(this.approachAnim, 3) / 3;
        const ap = 0.5 * (1 - t) + 0.02;
        const lX = lx - ap, rX = lx + ap, lY = q.fn(lX), rY = q.fn(rX);
        if (isFinite(lY) && isFinite(rY)) {
            ctx.save();
            const ls = sx(lX), lt = sy(lY), rs = sx(rX), rt = sy(rY);
            ctx.fillStyle = 'rgba(124,92,231,0.9)'; ctx.beginPath(); ctx.arc(ls, lt, 6, 0, Math.PI * 2); ctx.fill();
            ctx.strokeStyle = 'rgba(124,92,231,0.7)'; ctx.lineWidth = 2.5;
            ctx.beginPath(); ctx.moveTo(ls + 9, lt); ctx.lineTo(ls + 18, lt); ctx.stroke();
            ctx.fillStyle = 'rgba(255,217,61,0.9)'; ctx.beginPath(); ctx.arc(rs, rt, 6, 0, Math.PI * 2); ctx.fill();
            ctx.strokeStyle = 'rgba(255,217,61,0.7)';
            ctx.beginPath(); ctx.moveTo(rs - 9, rt); ctx.lineTo(rs - 18, rt); ctx.stroke();
        ...
        ctx.fillText('What y-value does the curve approach at the pink \u25cb?', w / 2, pad - 10);
        ctx.textAlign = 'left';
        // Labels
        ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.font = '13px "SF Mono", monospace';
        ctx.fillText(q.label, pad + 5, pad + gH + 35);
        ctx.fillStyle = 'rgba(255,107,157,0.8)'; ctx.font = 'bold 12px "SF Mono", monospace';
        ctx.fillText(q.sublabel, pad + 5, pad + gH + 52);
        ctx.fillStyle = '#ff6b9d'; ctx.font = 'bold 18px sans-serif'; ctx.textAlign = 'center';
